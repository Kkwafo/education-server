...
public class CodeValidationTestUtil {
     ...
     */
    public static final String DO_WHILE_PATTERN = "do\\s*\\{\\s*function\\s+[a-zA-Z_][a-zA-Z0-9_]*\\s*\\([^)]*\\)\\s*\\{[^}]*\\}\\s*\\s*(let|var|const)\\s+[a-zA-Z_][a-zA-Z0-9_]*\\s*=\\s*[^;]+;\\s*\\}\\s*while\\s*\\([^)]*\\);";
